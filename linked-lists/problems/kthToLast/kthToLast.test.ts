...
import { findKthToLastIteratevly, findKthToLastRecursivelyMain }  from './kthToLast'
...
it('[Recursively] Find kth to last elements of a linked list.', () => {
  let array = [1, 3, 4, 5, 2, 6, 7, 8]
  let node: Node = new Node(array[0])
  for (let i = 1; i < array.length; i++) {
    node.appendToEnd(array[i]);
  }
  let result = findKthToLastRecursivelyMain(node, 2);
  expect(result.printHumanReadable()).toBe('7->8->null')
})

it('[Recursively] Handle 2 nodes linked list', () => {
  let array = [1, 3]
  let node: Node = new Node(array[0])
  for (let i = 1; i < array.length; i++) {
    node.appendToEnd(array[i]);
  ...
  let all = findKthToLastRecursivelyMain(node, 2);
  expect(all.printHumanReadable()).toBe('1->3->null')

  let secondHalf = findKthToLastRecursivelyMain(node, 1);
  expect(secondHalf.printHumanReadable()).toBe('3->null')
})

it('[Recusively] Handle out of bounds k', () => {
  let array = [1, 3, 4, 5, 2, 6, 7, 8]
  let node: Node = new Node(array[0])
  for (let i = 1; i < array.length; i++) {
    node.appendToEnd(array[i]);
  ...
  let result = findKthToLastRecursivelyMain(node, 9);
  expect(result.printHumanReadable()).toBe('1->3->4->5->2->6->7->8->null')

  let result2 = findKthToLastRecursivelyMain(node, 0);
  expect(result2.printHumanReadable()).toBe('1->3->4->5->2->6->7->8->null')
})
